...
Plot2D::Plot2D(QString _name, int _lines, float _min, float _max,
...
void Plot2D::commonConstruction(int _lines, float _min, float _max,
                                 QColor color1, QColor color2, int _num_points){
    min = _min;
    max = _max;
    range = (int)(max - min);
...
void Plot2D::setNumberOfPoints(const int &_value){
    if(_value < (width()-15))
      num_points = _value;
    else
      num_points = width()-15;
...
void Plot2D::draw(QPainter *painter){
...
void Plot2D::add_new_value(int _line, float data_value){
    if(_line >= lines)
        return;
...
void Plot2D::print(QPainter *painter){
...
void Plot2D::setMaxValue(const float &value){
    max = value;
    range = (int)(max - min);
    rangeF = max - min;
}

void Plot2D::setMinValue(const float &value){
    min = value;
    range = (int)(max - min);
    rangeF = max - min;
}
