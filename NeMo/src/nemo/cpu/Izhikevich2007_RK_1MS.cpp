...
#pragma omp parallel for default(shared)
	for(int nl=0; nl < nn; nl++) {
		...
		k1 = (k[nl]*(v - Vr[nl])*(v - Vt[nl]) - u + I)*oneOverC; 
		temp = v + 0.5*DT*k1;
		k2 = (k[nl]*(temp - Vr[nl])*(temp - Vt[nl]) - u + I)*oneOverC; 
		temp = v + 0.5*DT*k2;
		k3 = (k[nl]*(temp - Vr[nl])*(temp - Vt[nl]) - u + I)*oneOverC; 
		temp = v + DT*k3;
		k4 = (k[nl]*(temp - Vr[nl])*(temp - Vt[nl]) - u + I)*oneOverC; 
		v += DT*(k1 + 2.0*k2 + 2.0*k3 + k4)/6.0;

		k1 = a[nl]*(b[nl]*(v-Vr[nl]) - u);
		temp = u + 0.5*DT*k1;
		k2 = a[nl]*(b[nl]*(v-Vr[nl]) - temp);
		temp = u + 0.5*DT*k2;
		k3 = a[nl]*(b[nl]*(v-Vr[nl]) - temp);
		temp = u +     DT*k3;
		k4 = a[nl]*(b[nl]*(v-Vr[nl]) - temp);
		u += DT*(k1 + 2.0*k2 + 2.0*k3 + k4)/6.0;
